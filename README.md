...
## 📌 Project Motivation
This project was inspired by real-world Anti-Financial Crime challenges, where the objective isn’t just predicting fraud, but explaining the reasoning in a way that human analysts can trust and act upon. By bridging the gap between "black-box" models and manual review teams, we create an interpretable scoring framework that enhances operational efficiency.

## 📌 Problem Description
In the financial industry, identifying fraudulent credit card transactions is a critical challenge. Modern machine learning models provide high accuracy but often lack transparency, leaving risk analysts without clear reasoning for a flagged transaction.

**The Goal:** Build a high-precision fraud detection system that not only classifies transactions but also provides **Natural Language Explanations** for its decisions using Large Language Models (LLMs).

> ⚠️ **Dataset Acknowledgment:** This project uses a well-known public credit card dataset as a proxy. The goal is to demonstrate a robust end-to-end ML workflow, interpretability, and deployment in realistic fraud detection scenarios.
...
We use SHAP (SHapley Additive exPlanations) to identify the top 5 contributing factors for every flagging event. **This allows analysts to quickly understand the key drivers of a flagged transaction without diving into model internals.**
...
**One-line Quick Start (if image is built):**
`docker run -d -p 8000:8000 --env-file .env --name fraud-api fraud-detection-api`
...
- **Precision-Recall Optimization**: Successfully handled extreme class imbalance (0.17%) by prioritizing PR-AUC and optimizing decision thresholds (F1-score maximization).
- **Human-in-the-loop Design**: Demonstrated how Generative AI can serve as a powerful bridge between complex model outputs and final human decision-making.
- **End-to-End MLOps**: Built a complete pipeline from raw data ingestion to containerized interactive deployment.

---
*Developed as part of the ML Zoomcamp Capstone Project.*
